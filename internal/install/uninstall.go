...
func (u *Uninstaller) Uninstall(ctx context.Context, opts UninstallOptions) error {
	...
	if err := u.deps.StopDaemonService(ctx, daemonCfg); err != nil {
		return fmt.Errorf("stop daemon service: %w", err)
	}
	if err := u.deps.UninstallDaemonService(ctx, daemonCfg); err != nil {
		return fmt.Errorf("uninstall daemon service: %w", err)
	}
	if opts.WithMenubar {
		menubarCfg := MenubarServiceConfig(paths)
		if err := u.deps.StopMenubarService(ctx, menubarCfg); err != nil {
			return fmt.Errorf("stop menubar service: %w", err)
		}
		if err := u.deps.UninstallMenubarService(ctx, menubarCfg); err != nil {
